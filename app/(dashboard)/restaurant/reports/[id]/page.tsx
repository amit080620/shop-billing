...
export default async function RestaurantBillDetailPage({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div>
        <h1 className="text-lg font-bold tracking-tight text-foreground md:text-2xl">{table?.name} · #{order.order_number}</h1>
        ...
        <ul className="flex flex-col gap-1.5">
          {(payments ?? []).map((p) => (
            <li key={p.id} className="flex justify-between rounded-lg bg-brand-soft px-3 py-2 text-sm">
              <span className="text-brand-text">{t(paymentMethodLabel(p.payment_method))}</span>
