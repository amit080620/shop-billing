...
function whatsappConfirmLink(a: Appointment) {
  const digits = a.customerPhone.replace(/\D/g, "");
  const withCountryCode = digits.length === 10 ? `91${digits}` : digits;
  const message = `Hi ${a.customerName}, your appointment for ${a.serviceName} at ${a.time} is confirmed. See you then!`;
      ...
      {appointment.status !== "completed" && appointment.status !== "cancelled" && appointment.status !== "no_show" && (
        <div className="mt-2 flex flex-wrap gap-1.5">
          {appointment.status === "booked" && (
            <a
              href={whatsappConfirmLink(appointment)}
              target="_blank"
