...
function SubmitButton({ label, pendingLabel, hasError }: { label: string; pendingLabel: string; hasError: boolean }) {
  ...
  return (
    <button type="submit" disabled={pending} className={`btn-primary-sm ${justSaved ? "animate-save-success" : ""}`}>
      {pending ? pendingLabel : justSaved ? "Saved ✓" : label}
    </button>
  );
}
    ...
    <div className="flex flex-col gap-4">
      ...
      {showCategoryForm && (
        ...
        >
          ...
          <SubmitButton label={t("products.saveCategory")} pendingLabel={t("products.saving")} hasError={!!categoryState?.error} />
        </form>
        </div>
        </div>
      )}
