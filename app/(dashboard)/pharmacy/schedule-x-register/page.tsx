...
export default async function ScheduleXRegisterPage({
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        title="Schedule X register"
        subtitle="Narcotic/controlled substance sales — for the compliance record you're required to keep."
        icon={
      ...
      {rows.length === 0 ? (
        <EmptyState text="No Schedule X sales in this range. Mark a medicine's drug schedule as 'Schedule X' in Inventory if you sell any controlled substances." />
      ) : (
        <>
          <ExportRegisterButton rows={rows} />
            ...
            {rows.map((r) => (
              <li key={r.id} className="rounded-lg border border-border bg-surface shadow-sm px-3.5 py-3">
                ...
                <p className="text-xs text-muted">
                  Batch {r.batchNumber} · Qty {r.quantity} · Invoice #{r.invoiceNumber}
                </p>
                <p className="text-xs text-muted">
